...
const getStudentFromId = studentId => studentRecords.find(record => record.id == studentId)
...
const paidStudentsToEnroll = () => {
	var recordsToEnroll = studentRecords.filter(record => record.paid && !currentEnrollment.includes(record.id));

	var idsToEnroll = recordsToEnroll.map(record => record.id);
